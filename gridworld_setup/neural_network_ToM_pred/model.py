...
class CharNet(nn.Module):
    def __init__(
        ...
        self.encoder = nn.Sequential(
            ResNetBlock(num_input, 4, 1).double().to(device),
            ...
            nn.AvgPool2d(grid_size)
            .double()
            .to(device),  # [batch * num_step, output, 1]
        )
    ...
    def forward(self, obs: torch.tensor) -> torch.tensor:
        ...
        obs = obs.permute(
            0, 1, 2, 5, 3, 4
        )  # [batch, num_past, num_step, channel, H, W]
        ...
        for p in range(num_past):
            ...
            obs_past = obs_past.permute(
                1, 0, 2, 3, 4
            )  # [num_step, batch, channel, H, W]
            obs_past = obs_past.reshape(
                -1, num_channel, H, W
            )  # [batch * num_step, channel, H, W]
...
class MentalNet(nn.Module):
    def __init__(
        self, num_input: int, num_step: int, num_output: int, device: str = "cuda"
    ) -> None:
        ...
        self.last_conv = (
            nn.Conv2d(num_step * 32, num_output, kernel_size=3, padding=1, bias=False)
            .double()
            .to(device)
        )
    ...
    def forward(self, obs: torch.tensor) -> torch.tensor:
        ...
        outs, _ = self.convlstm(
            x
        )  # [batch, num_step, ouput, H, W] ## WARNING MEMORY SPACE!!
...
class PredNet(nn.Module):
    def __init__(
        ...
        self.charnet = CharNet(
            num_input=num_input,
            num_step=num_step_obs,
            num_output=num_output_char,
            grid_size=grid_size_obs,
            device=device,
        )
        self.mentalnet_traj = MentalNet(
            num_input=num_input,
            num_step=num_steps_demo_eval,
            num_output=num_output_mental,
            device=device,
        )
        ...
        self.encoder = nn.Sequential(
            ResNetBlock(channel_in, 8, 1).double().to(device),
            ResNetBlock(8, 16, 1).double().to(device),
    ...
    def forward(
        ...
        if num_past == 0:
            e_char = torch.zeros(
                (batch_size, self.num_output_char, H, W, num_channel),
                device=self.device,
            )
        else:
            ...
            e_char_concat = e_char_concat.repeat(
                1, 1, H, W
            )  # [batch, num_output_char, H, W]
            # print('echar after model', e_char.shape)

        # Current traj
        _, num_step, _, _, _ = current_traj.shape
        if num_step == 0:
            e_mental = torch.zeros(
                (batch_size, self.num_output_mental, H, W, num_channel)
            )
        else:
        ...
        x_concat = torch.cat(
            [e_char_concat, e_mental_concat, query_state_concat], axis=1
        )  # [batch, num_output_char + num_output_mental + num_channel, H, W, num_channel]
    ...
    def train(self, data_loader: DataLoader, optim: Optimizer) -> dict:
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            pred_action, e_char, e_mental, query_state = self.forward(
        ...
        dicts = dict(
            accuracy=action_acc / len(data_loader), loss=tot_loss / len(data_loader)
        )
        return dicts
