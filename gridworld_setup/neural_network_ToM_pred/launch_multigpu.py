...
for epoch in nb_epoch:
    for bs in batch_size:
        ...
        run_directory = (
            run_directory.replace(" ", "_").replace(".", "-").replace(":", "__")
        )
        mkdir_p(run_directory)

        job_file = os.path.join(run_directory, "expe.slurm")

        with open(job_file, "w") as fh:
            fh.writelines("#!/bin/bash\n")
            fh.writelines("#SBATCH --job-name=expe\n")
            fh.writelines("#SBATCH --output=" + run_directory + "expe_%j.out\n")
            fh.writelines("#SBATCH --error=" + run_directory + "expe_%j.out\n")
            fh.writelines("#SBATCH --ntasks-per-node=1\n")
            fh.writelines("#SBATCH --cpus-per-task=20\n")
            fh.writelines("#SBATCH --nodes=" + str(nb_nodes) + "\n")
            ...
            fh.writelines("module load pytorch-gpu/py3/2.0.0\n")

            fh.writelines("set -x\n")

            fh.writelines("python idr_accelerate.py\n")
