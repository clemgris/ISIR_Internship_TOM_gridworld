...
class BayesianLearner:
    def __init__(
        ...
        while not feasible:
            feasible = True
            # Generate env
            self.init_env(grid_size=grid_size, num_colors=num_colors)
            self.env.reset()
            ...
            for obj_idx in self.env.obj_idx:
                ...
                path = A_star_algorithm(
                    start=self.env.agent_pos, goal=obj_idx, grid=grid
                )
    ...
    def play(self, size: int = None) -> list:
        ...
        for _ in range(size):
            ...
            if self.save_render:
                ...
                self.render_beliefs.append(
                    np.repeat(np.repeat(beliefs_image, 20, axis=0), 20, axis=1)
                )
        ...
        if self.save_render:
            ...
            pil_images = [
                Image.fromarray(image.astype("uint8"), "RGB")
                for image in self.render_frames
            ]
            pil_images[0].save(
                gif_file,
                save_all=True,
            ...
            pil_images = [
                Image.fromarray(
                    image / (Shannon_entropy(1 / 4 * np.ones(4)) + 0.5) * 255
                )
                for image in self.render_beliefs
            ]
            pil_images[0].save(
                gif_file,
                save_all=True,
    ...
    def update_beliefs(self, obs: np.ndarray) -> None:
        ...
        for vis_j in range(0, self.receptive_field):
            for vis_i in range(0, self.receptive_field):
                ...
                # Goal (door)
                if (obs[vis_i, vis_j, :2] == np.array([4, self.goal_color])).all() and (
                    self.env.agent_pos != (abs_i, abs_j)
                ):
                    self.beliefs[abs_i, abs_j, :] = np.array([0, 0, 1, 0])
                # Subgoal (key)
                elif (
                    obs[vis_i, vis_j, :2] == np.array([5, self.goal_color])
                ).all() and (self.env.agent_pos != (abs_i, abs_j)):
                    self.beliefs[abs_i, abs_j, :] = np.array([0, 0, 0, 1])
                # Obstacle
                elif obs[vis_i, vis_j, 0] in [2, 4, 5] and (
                    self.env.agent_pos != (abs_i, abs_j)
                ):
    ...
    def compute_exploration_score(self, dir: int, pos: tuple) -> float:
        ...
        grid = self.env.grid.slice(
            topX, topY, self.receptive_field, self.receptive_field
        )
        for _ in range(dir + 1):
            grid = grid.rotate_left()
        vis_mask = grid.process_vis(
            agent_pos=(self.receptive_field // 2, self.receptive_field - 1)
        )
    ...
    def exploration_goal(self) -> int:
        ...
        if len(unexplored_pos[0]) > 0:
            ...
            dist = np.array(
                [
                    Manhattan_dist(self.env.agent_pos, pos)
                    for pos in zip(unexplored_pos[0], unexplored_pos[1])
                ]
            )
            ...
            # Obstacle grid
            grid = np.ones((self.env.width, self.env.height)) - np.all(
                self.beliefs == np.array([1.0, 0.0, 0.0, 0.0]).reshape(1, 1, -1), axis=2
            )
            ...
            while dist[dest_idx] < 10e10:
                ...
                if path is None:
                    ...
                    dest_idx = np.random.choice(argmin_set)
                    dest_pos = (
                        unexplored_pos[0][dest_idx],
                        unexplored_pos[1][dest_idx],
                    )
                    grid = np.ones((self.env.width, self.env.height)) - np.all(
                        self.beliefs
                        == np.array([1.0, 0.0, 0.0, 0.0]).reshape(1, 1, -1),
                        axis=2,
                    )
                    grid[dest_pos[0], dest_pos[1]] = 0
                else:
                    ...
                    mapped_actions = map_actions(
                        self.env.agent_pos, path[0][1], self.env.agent_dir
                    )
    ...
    def active_exploration_policy(self, forced: bool = False) -> int:
        ...
        scores[0] = self.compute_exploration_score(
            dir=(self.env.agent_dir - 1) % 4, pos=self.env.agent_pos
        )
        # Turn right
        ...
        if np.all(
            self.beliefs[next_pos[0], next_pos[1], :] == np.array([0, 1, 0, 0])
        ) or np.all(
            self.beliefs[next_pos[0], next_pos[1], :] == np.array([0, 0, 1, 0])
        ):  # Obstacle in front
            scores[2] = -1.0
        else:
            scores[2] = self.compute_exploration_score(
                dir=self.env.agent_dir, pos=next_pos
            )
    ...
    def policy(self):
        ...
        if not self.transitions.empty():
            ...
            if not (
                (
                    np.all(
                        self.beliefs[pos_dest[0], pos_dest[1], :]
                        == np.array([0, 1, 0, 0])
                    )
                )
                or (
    ...
    def observe(self, traj: list, render_mode: str | None = "rgb_array") -> None:
        ...
        assert (self.env.agent_pos == self.env.agent_start_pos) & (
            self.env.agent_dir == self.env.agent_start_dir
        )
        ...
        for a in demo:
            ...
            if render_mode == "rgb_array":
                ...
                beliefs_image = Shannon_entropy(self.beliefs, axis=2) / (
                ...
                image[self.env.agent_pos[0], self.env.agent_pos[1]] = np.array(
                    [OBJECT_TO_IDX["agent"], 0, 0]
                )
