...
if __name__ == '__main__':
    ...
    val_storage = Storage(args.data_filename,
                       data_mode='val',
                       max_length_obs=max_length_obs,
                       max_length_demo_eval=max_length_demo_eval)
    val_data = val_storage.extract()

    test_storage = Storage(args.data_filename,
                       data_mode='test',
                       max_length_obs=max_length_obs,
                       max_length_demo_eval=max_length_demo_eval)
    ...
    prednet, optimizer, train_loader, val_loader, test_loader = accelerator.prepare(prednet, optimizer, train_loader, val_loader, test_loader)
    ...
    for epoch in range(n_epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader)):
            ...
            pred_action, e_char, e_mental, query_state = prednet(past_traj, current_traj, query_state)
        ...
        train_dict = dict(accuracy=action_acc / len(train_loader),
        ...
        for i, batch in enumerate(tqdm(val_loader)):
            ...
            action_acc_val += (torch.sum(pred_action_ind == target_action).item() / len(target_action))
        
        eval_val_dict = dict(accuracy=action_acc_val / len(val_loader),
    ...
    for i, batch in enumerate(tqdm(test_loader)):
        ...
        action_acc_test += (torch.sum(pred_action_ind == target_action).item() / len(target_action))
    
    eval_test_dict = dict(accuracy=action_acc_test / len(test_loader),
