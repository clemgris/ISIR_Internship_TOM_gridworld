...
class RegNet(nn.Module):
    ...
    def forward(self, past_traj: torch.tensor, demo: torch.tensor) -> tuple:
        ...
        if num_past == 0:
            e_char = torch.zeros(
                (batch_size, self.num_output_char, H, W), device=self.device
            )
        else:
        ...
        x_concat = torch.cat(
            [e_char_concat, e_mental_concat], axis=1
        )  # [batch, num_output_char + num_output_mental, H, W]
    ...
    def train(self, data_loader: DataLoader, optim: Optimizer) -> dict:
        ...
        dicts = dict(metric=metric / len(data_loader), loss=tot_loss / len(data_loader))
        return dicts
