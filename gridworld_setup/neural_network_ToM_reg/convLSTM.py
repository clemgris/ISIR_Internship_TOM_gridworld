...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input, hidden_state):
        ...
        ingate, remembergate, cellgate, outgate = torch.split(gates, self.hidden_channels, dim=1)
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input, hidden_states=None):
        if hidden_states is None:
            hidden_states = [(torch.zeros(input.size(0), ch, input.size(3), input.size(4)).to(input.device),
                              torch.zeros(input.size(0), ch, input.size(3), input.size(4)).to(input.device))
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_length):
                h, c = self.cell_list[layer_idx](input=cur_layer_input[:, t, :, :, :], hidden_state=(h, c))
