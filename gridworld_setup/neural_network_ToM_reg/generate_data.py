...
def parse_args():
    ...
    parser.add_argument(
        "--save_folder", "-save", type=str, default="./neural_network_ToM_reg/data"
    ),
...
if __name__ == "__main__":
    ...
    print(
        f"Save dataset config in {f'{args.save_folder}/dataset_{date}/dataset_config.json'}"
    )
                ...
                for rf_idx, receptive_field in enumerate(rf_values_demo):
                    for demo_goal_color in range(args.num_colors):
                        for demo_idx, demo_rf in enumerate(rf_values_demo):
                            ...
                            while (
                                not learner.terminated
                                and learner.env.step_count < learner.env.max_steps
                            ):
                                grid = learner.env.grid.slice(
                                    0, 0, args.grid_size, args.grid_size
                                )
                                image = grid.encode(vis_mask)
                                image[
                                    learner.env.agent_pos[0], learner.env.agent_pos[1]
                                ] = np.array([OBJECT_TO_IDX["agent"], 0, 0])
                            ...
                            demo = generate_demo(learner.env, demo_rf, demo_goal_color)
                            if (demo_rf == args.grid_size_demo) and (
                                np.random.uniform() > 0.5
                            ):
                                demo = generate_demo_all(learner.env)

                            # Learner observes the demonstration
                            learner.observe(demo, render_mode=None)
                            images_demo = learner.render_frames_observation

                            # Add first unused action to have first obs
                            if len(demo) > 0:
                                demo = [4] + demo
                            ...
                            vis_mask = np.ones(
                                (args.grid_size_demo, args.grid_size_demo)
                            )
                            while (
                                not learner.terminated
                                and learner.env.step_count < learner.env.max_steps
                            ...
                            with open(saving_filename, "wb") as f:
                                pickle.dump(
                                    (
                                        [
                                            (images_obs_env, actions_obs_env),
                                            (images_demo, demo),
                                        ],
                                        reward,
                                    ),
